...
import { getSingleRentBill } from "../../api/utilityApi";
...
export default function RentBillView() {
  ...
  const fetchBill = async () => {
    ...
    setError(null);
    try {
      const res = await getSingleRentBill(utilityId);
      setBill(res.data);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to load rent bill');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="p-4 sm:p-6 max-w-screen-xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            <h2 className="text-base font-semibold text-[#1E293B] mb-4 flex items-center gap-2">
              <TrendingUp size={18} className="text-[#002365]" />
              Rent Summary
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-[#F8FAFC] rounded-lg p-4 text-center">
                <p className="text-xs font-medium text-[#64748B] uppercase tracking-wide mb-1">Rent Amount</p>
                <p className="text-2xl font-bold text-[#1E293B]">{formatCurrency(bill.rentAmount)}</p>
                <p className="text-xs text-[#64748B] mt-1">base rent</p>
              </div>
              <div className="bg-[#F8FAFC] rounded-lg p-4 text-center">
                <p className="text-xs font-medium text-[#64748B] uppercase tracking-wide mb-1">Arrears</p>
                <p className="text-2xl font-bold text-amber-600">{formatCurrency(bill.arrears)}</p>
                <p className="text-xs text-[#64748B] mt-1">previous dues</p>
              </div>
              <div className="bg-[#002365] rounded-lg p-4 text-center">
                <p className="text-xs font-medium text-white/70 uppercase tracking-wide mb-1">Total Amount</p>
                <p className="text-2xl font-bold text-white">{formatCurrency(bill.totalAmount)}</p>
                <p className="text-xs text-white/70 mt-1">rent + arrears</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            ...
            <div className="space-y-1">
              <AmountRow
                label="Rent Amount"
                value={formatCurrency(bill.rentAmount)}
              />
              <AmountRow
                label="Arrears (Previous Dues)"
                value={formatCurrency(bill.arrears)}
                isNegative={bill.arrears > 0}
              />
              <AmountRow
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#64748B]">Rent Amount</span>
                <span className="text-sm font-semibold text-[#1E293B]">{formatCurrency(bill.rentAmount)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#64748B]">Arrears</span>
                <span className={`text-sm font-semibold ${bill.arrears > 0 ? 'text-amber-600' : 'text-[#1E293B]'}`}>
                  {formatCurrency(bill.arrears)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#64748B]">Paid Amount</span>
                <span className="text-sm font-semibold text-emerald-600">
                  {formatCurrency((bill.totalAmount || 0) - (bill.remainingAmount || 0))}
                </span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            ...
            <div className="space-y-2">
              <button
                onClick={() => navigate(`/manager/payments/new?utilityId=${bill.id}&type=Rent`)}
