...
import { getRentBills } from "../../api/utilityApi";
...
export default function RentBillsList() {
  ...
  const fetchBills = async () => {
    ...
    try {
      ...
      const res = await getRentBills(params);
      ...
      setError(err.response?.data?.message || 'Failed to load rent bills');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <button
          onClick={() => navigate(`/manager/rent/view/${row.id}`)}
    ...
    <div className="p-4 sm:p-6 max-w-screen-xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-[#1E293B]">Rent Bills</h1>
        </div>
        <p className="text-sm text-[#64748B]">
          View and manage all rent utility bills. Filter by tenant or billing month.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-12 flex flex-col items-center justify-center">
          <Spinner size="lg" />
          <p className="text-sm text-[#64748B] mt-3">Loading rent bills...</p>
        </div>
      ) : (
        <>
          {/* Results Count */}
          ...
          <div className="md:hidden space-y-4">
            {bills.length === 0 ? (
              <div className="bg-white rounded-xl border border-[#E2E8F0] p-8 text-center">
                <p className="text-sm text-[#64748B]">No rent bills found</p>
              </div>
            ) : (
              bills.map((bill) => (
                <div
                ...
                >
                  ...
                  <div className="flex items-center justify-end pt-2 border-t border-[#F1F5F9]">
                    <button
                      onClick={() => navigate(`/manager/rent/view/${bill.id}`)}
                      className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-[#002365] bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"
                    >
                      <Eye size={13} />
                      View
