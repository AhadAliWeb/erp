...
import { getSingleWaterBill } from "../../api/utilityApi";
...
export default function WaterBillView() {
  ...
  const fetchBill = async () => {
    ...
    setError(null);
    try {
      const res = await getSingleWaterBill(utilityId);
      setBill(res.data);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to load water bill');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            ...
            <div className="space-y-1">
              <AmountRow
                label="Water Charges (Units × Rate)"
                value={formatCurrency(bill.waterCharges)}
              />
              <AmountRow
                label="Fixed Charges"
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-[#1E293B]">
                  {bill.totalAmount > 0 && bill.waterCharges > 0
                    ? `${((bill.sstCharges / bill.waterCharges) * 100).toFixed(1)}%`
                    : '—'}
                </span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-5 sm:p-6">
            ...
            <div className="space-y-2">
              <button
                onClick={() => navigate(`/manager/payments/new?utilityId=${bill.id}&type=Water`)}
